...
public class UserController implements Initializable, IObserver
{
    ...
    public void bBook()
    {
        ...
        for (String clientIdText : clientIds)
        {
            ...
            final int clientIdFinal = clientId;
            Optional<Client> client = server.clientFindAll().stream()
                    .filter(c -> c.getId().equals(clientIdFinal))
                    .findFirst();
            ...
            if (client.isPresent())
            {
                Ticket ticket = new Ticket(0, client.get(), flight.get());
                server.ticketAdd(ticket);
            ...
            {
                logger.debug("Client with ID " + clientId + " not found.");
            }
        }
